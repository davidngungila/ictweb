    ...
    <div class="max-w-2xl">
        <div class="bg-white shadow rounded-lg">
            <div class="px-6 py-4 border-b border-gray-200">
                <h1 class="text-xl font-semibold text-gray-900">Edit Pricing Plan: {{ $pricingPlan->name }}</h1>
            </div>
            
            <form action="{{ route('admin.pricing.update', $pricingPlan) }}" method="POST" class="px-6 py-4">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="3" required
                                  class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:ring-blue-500 focus:border-blue-500 sm:text-sm">{{ old('description', $pricingPlan->description) }}</textarea>
                        @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <select name="currency" id="currency" required
                            ...
                            <option value="TZS" {{ old('currency', $pricingPlan->currency) === 'TZS' ? 'selected' : '' }}>TZS</option>
                    ...
                    <div>
                        ...
                        <select name="billing_cycle" id="billing_cycle" required
                            ...
                            <option value="one-time" {{ old('billing_cycle', $pricingPlan->billing_cycle) === 'one-time' ? 'selected' : '' }}>One-time</option>
                        </select>
                        @error('billing_cycle')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <textarea name="features" id="features" rows="5" required
                                  class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                                  placeholder="Feature 1&#10;Feature 2&#10;Feature 3">{{ old('features', is_array($pricingPlan->features) ? implode("\n", $pricingPlan->features) : $pricingPlan->features) }}</textarea>
                        @error('features')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="stripe_price_id" id="stripe_price_id"
                               ...
                               value="{{ old('stripe_price_id', $pricingPlan->stripe_price_id) }}">
